...
| Directional cap (90%) *(repo)* | Built in `execution/engine.py` (`DIRECTIONAL_CAP_PCT`); distinct from `master/strategist.py`'s `PER_BOT_WEIGHT_CEILING` (0.65) — see Stack 3. Raised from 0.60 on 2026-08-05 — 0.60 was inconsistent with the documented Track A capital allocation below (~93% deployable), causing valid trades to be rejected once Track A held its allocated capital. | 2026-08-05 |
