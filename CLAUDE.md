...
| Directional cap (60%) *(repo)* | Built in `execution/engine.py` (`DIRECTIONAL_CAP_PCT`); distinct from `master/strategist.py`'s `PER_BOT_WEIGHT_CEILING` (0.65) — see Stack 3. Briefly raised to 0.90 on 2026-08-05 under an incorrect hypothesis (that legitimate bot exposure needed the headroom); root cause was actually a single UNATTRIBUTED legacy position dominating the sum, fixed same-day by excluding `bot_source='UNATTRIBUTED'` in `check_directional_cap()`. Reverted to 0.60. | 2026-08-05 |
