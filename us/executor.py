"""
US Flow Bot — Executor (18:10 UTC, after us-trader's 18:05 ORB fire)
Reads shared/us-candidates.json, computes sizing from directives, writes
trade requests to nwt_tickets. Does NOT place orders — that is the
Execution Engine's role.

This closes a gap where the US bot (rewritten as a pure signal generator —
trade_1400_with_brackets.py — per the "US bot loses execution authority"
rebuild note) wrote candidates to us-candidates.json but nothing ever
turned them into TRADE_REQUEST tickets: no us/executor.py existed, and
master/strategist.py only classifies regime and sets bot-level
capital_weight, it never reads candidate files. Unlike that gap, EU/AUS/
China already had this executor role filled — this mirrors ukeu/executor.py.

Capital base: $35,000 allocated to US bot (largest single allocation).
time_in_force: 'day' always — Alpaca gotcha: GTC is required for ASX/UKEU
multi-day holds, but US ORB positions are intraday-to-5-day and day orders
are the documented US convention (CLAUDE.md Known Gotchas).
"""
    ...
    for candidate in candidates:
        try:
            ...
            ticket_payload = {
                ...
                "size_cap": size_cap,
                "time_in_force": "day",  # US: day orders only (CLAUDE.md gotcha)
