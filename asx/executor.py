"""
AUS Dividend/Momentum Bot — Executor (09:30 UTC)
Reads aus-candidates.json, computes sizing from directives, writes trade
requests to nwt_tickets. Does NOT place orders.

Capital base: $20,000 allocated to AUS bot.
time_in_force: 'gtc' (1-8 week hold, multi-day positions).
No options — ASX options liquidity too thin (architecture rule).
"""
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [AUS-EXEC] %(levelname)s %(message)s",
    datefmt="%Y-%m-%dT%H:%M:%SZ",
)
log = logging.getLogger("aus_executor")

AUS_CAPITAL_BASE = 20_000.0  # architecture spec: $20k allocated to AUS bot
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                "VALUES (%s, %s, %s, %s)",
                (level, "AUS_EXECUTOR", message, json.dumps(payload) if payload else None),
            )
        conn.commit()
...
def write_ticket(conn, payload: dict) -> str:
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            ("AUS_EXECUTOR", "EXECUTION_ENGINE", "TRADE_REQUEST", json.dumps(payload)),
        )
...
# ---------------------------------------------------------------------------
def main() -> None:
    log.info("AUS executor starting (09:30 UTC)")
    ...
    if directives.get("global_kill_switch", True):
        log.info("Global kill switch active — AUS executor exiting without tickets")
        return

    aus_perm = directives.get("bot_permissions", {}).get("aus", {})
    if aus_perm.get("status") == "paused":
        log.info("AUS bot status=paused — executor exiting without tickets")
        return

    capital_weight = float(aus_perm.get("capital_weight", 0.0))
    size_cap = float(aus_perm.get("size_cap", 0.0))

    if capital_weight <= 0 or size_cap <= 0:
        log.info("AUS capital_weight=%.2f or size_cap=%.2f is zero — no tickets", capital_weight, size_cap)
        return

    # Step 2: Read candidates
    if not CANDIDATES_FILE.exists():
        log.info("aus-candidates.json not found — no candidates to process")
        return
    ...
    if not candidates:
        log.info("aus-candidates.json is empty — no tickets to write")
        return
    ...
    for candidate in candidates:
        # Hard guard: AUS bot never submits options tickets
        if candidate.get("asset_type", "equity") == "option":
            log.error(
                "ISOLATION VIOLATION: AUS executor received an options candidate for %s — rejected. "
                "ASX options are explicitly disallowed (liquidity too thin).",
                candidate.get("symbol"),
            )
            log_to_db(conn, "ERROR", f"AUS options isolation violation rejected: {candidate.get('symbol')}")
            continue
        ...
        try:
            ...
            ticket_payload = {
                ...
                # off the ticket payload directly, preferring it over genome/
                # hardcoded defaults. Leaving these only nested meant the
                # strategist's actual per-signal exit levels were silently
                # discarded at fill time.
                "stop_pct": candidate.get("expected_payoff", {}).get("stop_pct"),
                "target_pct": candidate.get("expected_payoff", {}).get("target_pct"),
                "rationale": candidate.get("rationale", ""),
                "generated_at": candidate.get("generated_at"),
                "bot_source": "AUS_BOT",
                "asset_type": "equity",  # AUS: equities only
            ...
            log_to_db(conn, "ERROR", f"AUS ticket write failed for {candidate.get('symbol')}: {exc}")

    log_to_db(conn, "INFO", f"AUS executor wrote {tickets_written} ticket(s)", {
        "tickets": tickets_written,
        "capital_weight": capital_weight,
        "size_cap": size_cap,
    })
    log.info("AUS executor done — %d ticket(s) written", tickets_written)
    conn.close()
