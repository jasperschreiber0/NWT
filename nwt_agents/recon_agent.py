    ...
    if match:
        with conn.cursor() as cur:
            cur.execute(
                ...
                (
                    symbol,
                    "option" if apos.get("asset_class") == "us_option" else "equity",
                    apos["side"],
                    apos["qty"] * apos["avg_entry"],
                    apos["qty"],
                    apos["avg_entry"],
            ...
            cur.execute(
                ...
                INSERT INTO nwt_portfolio_ledger
                  (bot_source, asset, asset_type, direction, notional_risk,
                   entry_price, entry_time, status, lifecycle_state)
                VALUES ('UNATTRIBUTED', %s, %s, %s, %s, %s, NOW(), 'open', 'OPEN')
                """,
