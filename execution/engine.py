...
# ---------------------------------------------------------------------------
# Shortability gate (defense-in-depth)
# ---------------------------------------------------------------------------
#
# Root cause of the recurring EWU 422s: a short-direction signal can be
# generated by a strategist (e.g. ukeu/strategist.py) for an asset Alpaca
# will never let this account short (shortable=False / hard-to-borrow). That
# strategist-side gate is the primary defense — it must never emit the
# candidate in the first place. This is the second, independent check: if a
# short equity ticket reaches the engine anyway (stale strategist-side
# cache, a bug, a manual ticket), it must be rejected here too, and
# classified distinctly as STRUCTURALLY_IMPOSSIBLE rather than a generic
# FAILED — this was never a broker/network/sizing failure, it was never
# tradeable, and conflating the two corrupts the learning system's
# attribution (see CLAUDE.md "Do Nothing as a First-Class State").
#
# Shortability rarely changes, so this is cached to a shared JSON file
# (shared/asset-shortability-cache.json — same shared/ directory Track A
# bots already use for candidates.json / master-directives.json) with a
# bounded TTL, never queried on every ticket.
...
def _save_shortability_cache(cache: dict) -> None:
    try:
        ...
        logger.warning("Failed to write shortability cache: %s", exc)


def get_shortability(symbol: str) -> dict | None:
    """
    {'shortable': bool, 'easy_to_borrow': bool, 'borrow_status': str} for
    `symbol`, from Alpaca's own /v2/assets/{symbol} — the same asset
    metadata Alpaca itself checks before rejecting a short order with a 422.
    Cached with a bounded TTL (SHORTABILITY_CACHE_TTL_HOURS) so borrowability
    can genuinely change (e.g. a hard-to-borrow asset becoming shortable
    again) without ever hitting the API on every ticket. Returns None only
    if there is no usable cache entry AND the live lookup also failed —
    callers must treat None as "unknown", never as "shortable".
    """
    ...
    try:
        data = alpaca_get(f"/assets/{symbol}")
        fresh = {
        ...
        logger.warning("Shortability lookup failed for %s: %s", symbol, exc)
...
def process_ticket(conn, ticket: dict, directives: dict) -> None:
    ...
    # Defense-in-depth shortability gate — see "Shortability gate" section
    # above. The strategist-side gate is primary; this exists in case a
    # short equity ticket reaches the engine anyway.
    if asset_type == "equity" and direction == "short":
        shortability = get_shortability(symbol)
        if shortability is None or not shortability.get("shortable", False):
            reason = (
                f"Structurally impossible: {symbol} is not shortable on Alpaca "
                f"(shortable={shortability.get('shortable') if shortability else 'unknown'}, "
                f"borrow_status={shortability.get('borrow_status') if shortability else 'unknown'}) "
                "— defense-in-depth check; this should have been filtered upstream by the strategist"
            )
            logger.warning("Ticket %s: %s", ticket_id, reason)
