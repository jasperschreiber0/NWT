...
def link_decision_ticket(conn, strategy_id: str, symbol: str, genome_version, run_date, ticket_id: str) -> None:
    """
    Set ticket_id on the nwt_decision_inputs row the strategist wrote for
    this candidate — strategist and executor are separate daily processes,
    so the observation exists before the ticket does. Exact-match lookup on
    the same (strategy_id, genome_version, symbol, run_date) idempotency key
    every canonical decision row uses — never a fuzzy timestamp/symbol
    search. No-op if the row is already linked (or was never written, e.g.
    a candidate from before this migration).
    """
    try:
        with conn.cursor() as cur:
            cur.execute(
